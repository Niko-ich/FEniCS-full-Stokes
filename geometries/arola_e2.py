...
class ArolaE2(Geometry):
	def __init__(self,length_x=5000,resolution=16,dimension=2,num_more_grid=3,time_dependent=False):
		...
		for k in range(0,len(self.initial_domain[1][:])):
			...
			print('coordinates',[self.initial_domain[0][k],self.initial_domain[1][k]])
			...
			if(k<len(self.initial_domain[2][:])-1):
				for i in range(1,num_more_grid):
					...
					if(self.initial_domain[0][k]>=2200 and self.initial_domain[0][k]<2500):
						domain_vertices.append(Point(x_left+(i-1)/num_more_grid *(x_right-x_left),
								  	z_left+i/num_more_grid*(z_right-z_left)))
						self.bottom_grid.append([x_left+(i-1)/num_more_grid *(x_right-x_left),
								  	z_left+i/num_more_grid*(z_right-z_left)])
					...
					if(self.initial_domain[0][k]>=2200 and self.initial_domain[0][k]<2500):
						domain_vertices.append(Point(x_left+i/num_more_grid *(x_right-x_left),
								  	z_left+(i+1)/num_more_grid*(z_right-z_left)))
						self.bottom_grid.append([x_left+i/num_more_grid *(x_right-x_left),
								  	z_left+(i+1)/num_more_grid*(z_right-z_left)])
		...
		for k in range(0,len(self.initial_domain[2][:])):
			...
			#self.top_grid.append([self.initial_domain[0][len(self.initial_domain[2][:])-1-k],self.initial_domain[2][len(self.initial_domain[2][:])-1-k]])
		...
		for i in range(0,np.shape(bmesh.coordinates())[0]):
			...
			if(k>0):
				...
				if (bmesh.coordinates()[i][1] > l1*self.initial_domain[2][k]+l2*self.initial_domain[2][k-1]-0.2):
					# We do not add the [5000.0, 0.0] bottom point.
					if(time_dependent== True):
		...
		def bottom_part(x, on_boundary):
			# First, we find the index corresponding to the boundary
			k = 0
			while(self.initial_domain[0][k]< x[0]):
				k = k + 1
			# We have this case, because we can not index the -1 element.
			if (k>0):
				length = self.initial_domain[0][k] - self.initial_domain[0][k-1]
				l1 = 1-(self.initial_domain[0][k] - x[0]) / length
				l2 = 1-(x[0] - self.initial_domain[0][k-1]) / length
				return on_boundary and x[1] < l1*self.initial_domain[1][k]+l2*self.initial_domain[1][k-1]+0.2 and (x[0] <= 2200.0 or x[0] >= 2500.0)
			else:
				return on_boundary and (x[0] <= 2200.0 or x[0] >= 2500.0)
			
		def bottom_second_part(x, on_boundary):
			# First, we find the index corresponding to the boundary
			k = 0
			while(self.initial_domain[0][k]< x[0]):
				k = k + 1
			...
			if (k>0):
				...
				l1 = 1-(self.initial_domain[0][k] - x[0]) / length
				l2 = 1-(x[0] - self.initial_domain[0][k-1]) / length
				return on_boundary and x[1] < l1*self.initial_domain[1][k]+l2*self.initial_domain[1][k-1]+1.0 and x[0] >= 2200.0-5.0 and x[0] <= 2500.0+5.0
			else:
				return on_boundary and x[0] >= 2200.0-5.0 and x[0] <= 2500.0+5.0

		self.boundaries = [bottom_part,bottom_second_part]

	def set_boundary_conditions(self,V):
		# We set the boundary conditions corresponding to the ISMIP-HOM experiment E1
		noslip = Constant((0.0,0.0))
		noslip_part = Constant(0.0)
		bc_bottom = DirichletBC(V,noslip,self.boundaries[0])
		bc_bottom_second = DirichletBC(V.sub(1),noslip_part,self.boundaries[1])
		return [bc_bottom,bc_bottom_second]
