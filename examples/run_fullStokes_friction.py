...
for friction_coefficient in friction_coefficients:
	...
	initial_stokes = Stokes(Domain,Elements,B*realistic_factor,f,friction_domain=Domain.ds(0),friction_coefficient=friction_coefficient)
	initial_solve = LinearDirect(initial_stokes)
	initial_solve.solve()
	equation.U = initial_stokes.U
	line_search = ConstantStep()
	solver = Picard(equation,line_search,max_iter=32,calc_norm=True,output_file='friction_corrected2_residual/150x30Picard_Reference_friction_coeff_high_res'+str(friction_coefficient))
	...
	solver = Newton(equation,line_search,max_iter=32,calc_norm=True,
				output_file='friction_corrected2_residual/150x30Newton_with_Armijo_min_step_friction_coeff_high_res'+str(friction_coefficient))#,ref_solution=solution_picard_ref)
	...
	solver = Newton(equation,line_search,max_iter=32,calc_norm=True,
				output_file='friction_corrected2_residual/150x30Newton_with_exact_friction_coeff_high_res'+str(friction_coefficient))#,ref_solution=solution_picard_ref)
	...
	equation.U = initial_stokes.U
	line_search =Exact(min_term='functional',right=4.0,left=0.0)
	solver = Picard(equation,line_search,max_iter=32,calc_norm=True,
	output_file='friction_corrected2_residual/150x30Picard_exact_friction_coeff_high_res'+str(friction_coefficient))#,ref_solution=solution_picard_ref)
	...
	solver = Picard(equation,line_search,max_iter=10,calc_norm=True,
	output_file='friction_experiment/Picard_armijo_friction_coeff'+str(friction_coefficient),ref_solution=solution_picard_ref)
